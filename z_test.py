...
def create_tab_z_test(parent_notebook):
    ...
    def calculate_population_mean_deviation():
        d = [(x-calculate_sample_average())**2 for x in get_input()]
        dispersion = sum(d)/len(get_input())
    ...
    data_entry = scrolledtext.ScrolledText(frame, width=30, height=20)
    data_entry.grid(column= 0, row = 1, padx=10)

    label_for_population_mean = Label(frame, text= 'Enter the Population Mean (μ)')
    label_for_population_mean.grid(column= 1, row = 0)
    entry_for_population_mean = Entry(frame)
    ...
    entry_for_population_standard_deviation = Entry(frame)
    entry_for_population_standard_deviation.grid(column= 2, row = 1, sticky='N')

    calculate_data = Button(frame, width=35, text='Calculate sample average', command=calculate_sample_average)
    ...
    z_test = Button(frame, width=30, text='Z-test data', command=calculate_z_stat)
    z_test.grid(column= 1, row = 2)
    z_test_label = Label(frame)
    z_test_label.grid(column=1, row = 3)

    p_val_by_z = Button(frame, width=30, text='Calculate p-value from Z-test', command = calculate_p_val_z_stat)
    p_val_by_z.grid(column= 2, row = 2)
    p_val_by_z_label = Label(frame)
